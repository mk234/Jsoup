                ...
                try {
                    ...
                    for (var i = 0; i < frames.length; i++) {
                        if (frame === null || frames[i] == frame) {
                            ...
                            if (!found) {
                                GemiusPlayerVisibility.childs[GemiusPlayerVisibility.childs.length] = {
                                    'frame': frames[i],
                                    'vis': null
                                };
                            }
...
function GemiusPlayer(playerID, gemiusID, playerData) {
    ...
    this._getProgramParams = function (programID) {
        ...
        var pkeys = {
            'programType': '_SCTE',
            'programDuration': '_SCD',
            'programName': '_SCT',
            'series': '_SCS',
            'typology': '_SCTY',
            'premiereDate': '_SCPD',
            'externalPremiereDate': '_SCEPD',
            'quality': '_SCQ',
            "resolution": "_SCR",
            "volume": "_SCV"
        };
    ...
    this._getPlayerParams = function () {
        ...
        return params.concat(this._convertParams(this.playerData, pkeys));
    };
...
if (typeof window['gemius_player_data'] != 'undefined') {
    for (var i = 0; i < window['gemius_player_data'].length; i++) {
        ...
        if (fun != "GemiusPlayer") obj[fun].apply(obj, args);
            ...
            var nobj = new function () {
                return GemiusPlayer.apply(this, args);
            };
...
(function (d, t) {
    ...
    try {
        ...
        gt.async = 'true';
        gt.defer = 'true';
        gt.src = l + '://scz.hit.gemius.pl/gemiuslib.js';
