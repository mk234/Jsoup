...
if (typeof gemius_hcconn == "undefined") {
    gemius_hcconn = {
        ...
        hc: (typeof gemius_hitcollector != 'undefined') ? gemius_hitcollector : (typeof pp_gemius_hitcollector != 'undefined') ? pp_gemius_hitcollector : 'gacz.hit.gemius.pl',
        ...
        append_script: function (url, finishedfn, notify) {
            ...
            try {
                ...
                if (finishedfn != null) {
                    ...
                    gemius_hcconn.add_event(gt, 'readystatechange', function () {
                        if (!gt.readyState || gt.readyState === 'loaded' || gt.readyState === 'complete') finishedfn();
                    });
                }
                gemius_hcconn.add_event(gt, 'load', function () {
                    gemius_hcconn.remove_script(rndid, notify ? url : null);
                });
                gemius_hcconn.add_event(gt, 'error', function () {
                    gemius_hcconn.remove_script(rndid, null);
                });
                gemius_hcconn.add_event(gt, 'readystatechange', function () {
                    if (!gt.readyState || gt.readyState === 'loaded' || gt.readyState === 'complete') gemius_hcconn.remove_script(rndid, notify ? url : null);
                });
        ...
        setfpcookie: function () {
            ...
            if (gemius_hcconn.fpdata != '') {
                try {
                    document.cookie = "__gfp_64b=" + gemius_hcconn.fpdata + "; path=/" + ((gemius_hcconn.fpcdomain) ? ("; domain=" + gemius_hcconn.fpcdomain) : "") + "; expires=" + (new Date(exp)).toGMTString();
                } catch (e) {
                }
            }
        },
        ...
        addframe: function (ssl, file, args, rcvfn) {
            if (document.body) {
                ...
                setTimeout(function () {
                    gemius_hcconn.addframe(ssl, file, args, rcvfn);
                }, 100);
            }
        },
        frameto: function () {
        ...
        parameters: function () {
            var d = document;
            var w = window;
            var href = String(d.location.href);
            var ref;
            var f = 0;
            var dd;
            if (d.referrer) {
                ref = String(d.referrer);
            } else {
                ref = '';
            }
            if (typeof Error != 'undefined') {
            ...
            try {
                if (ref == "" && typeof ia_document != "undefined" && ia_document.referrer) ref = "https://" + (String(ia_document.referrer));
            } catch (e) {
            }
        ...
        internal_hit: function (allowaddscript, vers, id, evid, et, hsrc, sonar, extra) {
                ...
                if ((typeof gemius_test_sonar != 'undefined') || (typeof pp_gemius_test_sonar != 'undefined')) {
                    ...
                    if (cdata[0] >= 1) {
                        ...
                        for (var i = 0; i < addparams.length; i++) {
                            ...
                            extra += ((String(addparams[i])).replace(/\|/g, '_'));
                        }
                    }
                }
            }
            ...
            gemius_hcconn.requests[gemius_hcconn.requests.length] = {
                req: req,
                allowaddscript: allowaddscript,
                vers: vers
            };
            gemius_hcconn.latehits();
        },
        ...
        sonar_add: function (identifier, evid, freq, extra) {
            ...
            if (identifier && evid && freq > 0) {
                var fun = (function (sid) {
                    return function () {
                        gemius_hcconn.sonar(sid);
                    };
                }(gemius_hcconn.sonar_data.length));
        ...
        sonar_save: function () {
            ...
            for (var i = 0; i < gemius_hcconn.sonar_data.length; i++) {
                ...
                if (data["id"] && gemius_hcconn.fpcdomain) {
                    ...
                    try {
                        document.cookie = value + "; path=/; domain=" + gemius_hcconn.fpcdomain + "; expires=" + (new Date(exp)).toGMTString();
                    } catch (e) {
                    }
                }
            }
            ...
            try {
                ...
                for (var i = 0; i < cookies.length; i++) {
                    ...
                    if (cookie_arr[0].replace(/^\s+|\s+$/g, '') == '__gfp_' + id) {
                        ...
                        if (data.length == 6) {
                            if (del) try {
                                document.cookie = "__gfp_" + id + "=~; path=/; domain=" + data[5] + "; expires=Thu, 01 Jan 1970 00:00:01 GMT;";
                            } catch (e) {
                            }
        ...
        gevent: function (allowaddscript, vers, args, evid, hsrc, sonar) {
            ...
            var et = "view";
            if (args.length > 1) {
                var m = (String(args[0])).match('^_([a-zA-Z0-9]+)_$');
                if (m) {
                    et = m[1];
                    pos = 1;
                }
        ...
        addscripthit: function () {
            gemius_hcconn.ghit(1, 96, arguments, 0, 2, 0);
        },
        plainhit: function () {
            gemius_hcconn.ghit(0, 97, arguments, 0, 2, 0);
        },
        addscriptevent: function () {
            gemius_hcconn.gevent(1, 96, arguments, 0, 3, 0);
        },
        plainevent: function () {
            gemius_hcconn.gevent(0, 97, arguments, 0, 3, 0);
        },
        ...
        init: function () {
            ...
            if (gemius_hcconn.waiting_for_fpdata) {
                var domain = String(document.location.hostname);
            ...
            gemius_hcconn.add_event(window, "unload", function () {
                gemius_hcconn.unload(true);
            });
            gemius_hcconn.add_event(window, "beforeunload", function () {
                gemius_hcconn.unload(true);
            });
            gemius_hcconn.add_event(document, "mousedown", function () {
